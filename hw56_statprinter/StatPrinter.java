...
//Check small details ie. your size() +1s and size() -1s
//seriously messed me up
//tracing takes a while... so how to catch small mistakes like that
//on the exam without going crazy? mo practice?
...
public class StatPrinter {
	...
	//while loop goes through every index of freq
	int i = 0;
	while(i < max(data) +1){
	    //for loop goes through every index of data
	    //searches for all instances of i in data and sets freq accordingly
	    _frequency.add(0);
	...
	int maxx = 0;
	int Ctr = 0;
	//while loop goes through each index in ArrayList data
	while(Ctr < data.size()){
	    if(maxx < data.get(Ctr)){
		maxx = data.get(Ctr);
	    }
	    Ctr ++;
	}
	...
	//while loop goes through each index in frequency
	int Ctr = 0;
	while(Ctr < _frequency.size()){
	    System.out.print(Ctr + " : ");
	    //for loop prints the asterisks until the frequency num is reached
	    for(int i = 0; i < _frequency.get(Ctr)*ratio; i ++){
		System.out.print("* ");
	    }
	    System.out.println();
	    Ctr ++;
